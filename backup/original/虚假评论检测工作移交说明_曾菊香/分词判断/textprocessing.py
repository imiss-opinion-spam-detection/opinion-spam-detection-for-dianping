...
import jieba
import jieba.posseg
jieba.load_userdict('C:\Python27\Lib\site-packages\jieba\dict.txt') #Load user dictionary to increse segmentation accuracy
...
def get_excel_data(filepath, sheetnum, colnum, para):
    ...
    sheet = table.sheet_by_index(sheetnum-1)
...
def segmentation(sentence, para):# 结巴分词
    if para == 'str':
...
output: A multidimentional list 超前性列表
    [u'\u8fd9\u6b3e\u624b\u673a\u5927\u5c0f\u5408\u9002\uff0c',
    u'\u914d\u7f6e\u4e5f\u8fd8\u53ef\u4ee5\uff0c', u'\u5f88\u597d\u7528\uff0c',
    u'\u53ea\u662f\u5c4f\u5e55\u6709\u70b9\u5c0f\u3002', u'\u603b\u4e4b\uff0c',
    u'\u6234\u5983+\u662f\u4e00\u6b3e\u503c\u5f97\u8d2d\u4e70\u7684\u667a\u80fd\u624b\u673a\u3002']
...
def cut_sentence_2(words):
    ...
    for word in words:
        if word not in punt_list:
            ...
            i += 1
            token = list(words[start:i+2]).pop()# 移除最后一个值并返回其值
        else:
            sents.append(words[start:i+1])
            start = i+1
            i += 1
...
def seg_fil_excel(filepath, sheetnum, colnum):
    ...
    stopwords = get_txt_data(r'C:\Python27\Review-Helpfulness-Prediction-master\main\Preprocessing module\stopword.txt', 'lines')
...
def seg_fil_senti_excel(filepath, sheetnum, colnum):
    ...
    sentiment_stopwords = get_txt_data(r'C:\Python27\Review-Helpfulness-Prediction-master\main\Preprocessing module\sentiment_stopword.txt', 'lines')
